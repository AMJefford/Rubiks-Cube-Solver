class Face{
  ...
  //each face has colour and normal
  PVector normal;
  color c;
  ...
  //rotation vectors applied here
  //turning along x axes, x stays same, y and z change, apply matrix 
  ...
  void show(){
    ...
    //z is already forward facing but need to rotate x and y faces 
    if (abs(normal.x) > 0)
    {
      rotateY(HALF_PI);
    }
